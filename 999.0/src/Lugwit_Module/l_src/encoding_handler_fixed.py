...
# 创建简化的 LPrintHandler 类，用于替换 usualFunc_old.py 中的 _LPrintHandler
class LPrintHandler(object):
    """简化的 LPrint 处理器，使用独立的编码处理"""
    ...
    def emit(self, record):
        ...
        try:
            ...
            # 使用增强的输出函数
            EncodingHandler.enhanced_output(msg)
            
        except Exception:
            # 处理错误
            try:
                print("lprint handler error")
            except:
                pass
