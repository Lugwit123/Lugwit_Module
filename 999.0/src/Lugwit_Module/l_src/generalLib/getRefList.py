...
def readfile(findFileList='',startPos='',step='',index=0,file='',isEndQueue=''):
    print (u'process {} start to run,file--{}'.format(index,__file__))
    read_st=time.time()
    with open(file,'rb') as f:
        try:  # catch OSError in case of a one line file 
            ...
            f_read=str(f_read)
            # print (u'字符长度{}'.format(len(f_read)))
            # print (u'打开文件位置{},读取数量为{},花费时间B--{}--循环是{}'.format(startPos,step,time.time()-read_st,index))
            findList=[]
            if f_read:
                ...
                #isEndQueue[1]+=1
                if findList:
                    print (u'找到文件数量为{}'.format(len(findList)))
            else:
                print  (u'到第{}个进程查询完毕'.format(index))
...
def getRefFileListInMayaFile_MultiProcess(file):
    ...
    with multiprocessing.Manager() as  manager:
        ...
        print (u'多进程执行结束')
        print  (u'进程数量为{}'.format(len(allQueue)))
        #print (findFileList)
...
if __name__ == '__main__':        
    ...
    print(u'找寻到的所有列表为{},长度为{}'.format(list(enumerate(allList)),len(allList)))
    print (u'获取所有列表花费时间{}'.format(time.time()-st))